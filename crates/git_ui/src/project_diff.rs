...
impl ProjectDiff {
    ...
    pub fn move_to_entry(
        ...
        let order_by_path_only = self.branch_diff.read(cx).diff_base().has_explicit_head();
        let Some(git_repo) = self.branch_diff.read(cx).repo() else {
            return;
        };
        let repo = git_repo.read(cx);
        let path_key =
            project_diff_path_key(repo, &entry.repo_path, entry.status, order_by_path_only, cx);
    ...
    pub async fn refresh(this: WeakEntity<Self>, cx: &mut AsyncWindowContext) -> Result<()> {
        ...
        // A `BTreeMap` iterates in sorted (final display) order, so each excerpt
        // is appended below the previous one as it loads instead of being
        // inserted above already-visible content and shifting the view.
...
fn project_diff_path_key(
    ...
    repo_path: &RepoPath,
    status: FileStatus,
    order_by_path_only: bool,
    cx: &App,
) -> PathKey {
    let settings = GitPanelSettings::get_global(cx);
    // A PR review diff (explicit head) orders purely by path, ignoring the
    // status-based grouping, so the editor matches the review sidebar / GitHub.
    let sort_prefix = if order_by_path_only || settings.group_by != GitPanelGroupBy::Status {
        TRACKED_SORT_PREFIX
